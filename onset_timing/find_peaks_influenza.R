# find peaks for influenza time-series
...
seasons <- list(
  "2023-2024" = list(start = "2023-07-01", end = "2024-05-01"),
...
results <- purrr::map_dfr(names(seasons), function(season_name) {
  ...
  ed %>%
    ...
    slice_max(percent_visits_smoothed_influenza, n = 1, with_ties = FALSE) %>%
    ungroup() %>%
    transmute(season = season_name, hsa_nci_id, hsa, hsa_counties, geography,
              peak_date = week_end,
              peak_size= percent_visits_influenza)
})

write.csv(results, "peaks_influenza.csv", row.names = FALSE)
