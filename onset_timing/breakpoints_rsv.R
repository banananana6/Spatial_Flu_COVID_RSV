# find breakpoints through segmented regression, for rsv
...
locs=read.csv("ED.csv") %>% dplyr::filter(!is.na(percent_visits_rsv))  %>%  
...
ed <- ed %>%
  group_by(hsa_nci_id) %>%
  arrange(week_end) %>%
  mutate(cases_corrected_smoothed = rollmean(percent_visits_rsv, k = 3, fill = NA, align = "right"))
...
peak_dates <- ed %>%
  ...
  group_by(hsa_nci_id) %>%
  filter(max(percent_visits_rsv, na.rm = TRUE) > 0.5) %>%
  ...
  df %>% 
    ...
    mutate(
      ...
      segmented.analysis = map2(data, start.date,
                                ~ segmented(lm(percent_visits_rsv ~ days.from.start, data = .),
...
epidemic.start <- 
  find.breakpoints(ed %>% 
      ...
      subset(!(hsa_nci_id %in% c(533))) %>% # for 2024-2025/rsv
...
epidemic.start %>% 
  # filter(hsa_nci_id %in% c(482, 825, 190, 164, 204, 222, 827, 411, 894, 243)) %>% # 2023-2024 rsv (earliest 10 breakpoints)
  # filter(hsa_nci_id %in% c(532,263,518,470,893,408,305,821,517,109)) %>% # 2024-2025 rsv
  filter(hsa_nci_id %in% c(143,505,588,153,246,408,68,27,416,48)) %>% # 2025-2026 rsv
...
write.csv(time_data, "breakpoints_2025_2026_rsv.csv", row.names = FALSE)
