...
ed <- read.csv("ED.csv") %>%
  ...
  group_by(hsa_nci_id) %>%
  mutate(
    percent_visits_rsv_ma3 = zoo::rollmean(
      percent_visits_rsv,
      k = 3,
      fill = NA,
      align = "center"
...
peaks <- read.csv("peaks_rsv.csv", stringsAsFactors = FALSE) %>%
  mutate(
    hsa_nci_id = as.numeric(hsa_nci_id),
...
breakpoints <- read.csv("breakpoints_rsv.csv") %>%
...
g0 <- ggplot(subset(ed, hsa_nci_id %in% selected_ids)) +
  ...
  geom_line(aes(week_end, percent_visits_rsv_ma3, color = "RSV"), lwd = 0.8) +
